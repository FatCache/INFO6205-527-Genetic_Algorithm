...
// Calculate the best entity of the current generation
    ...
    // Generation a population using parents
    ...
    public CompletableFuture<TSPChromosome[]> parGenerate(TSPChromosome[] generatingList, int from, int to) {
        return CompletableFuture.supplyAsync(() -> {
            TSPChromosome[] tempList = new TSPChromosome[to - from + 1];
            TSPChromosome[] generatedList = new TSPChromosome[to - from + 1];

            System.arraycopy(generatingList, from, tempList, 0, to - from + 1);

            Arrays.sort(tempList);
            int flag;
            generatedList[0] = tempList[0];
            for (int k = 1; k < generatedList.length; k++) {
                // Select the best 10 percent
                flag = random.nextInt(tempList.length / 10);
                generatedList[k] = tempList[flag];
            }
            // To Mutate or To Crossover
